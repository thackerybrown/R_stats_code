---
title: "Necos, Right hemisphere, two hippoROIs*3 conds"
...
```{r}
testdata = read.xls("testfile1.xls")
head(testdata)
summary(testdata)
```
...
res1 = lmer(value ~ variable * cond + (1|sub), data=myrepeatedmeasures)
summary(res1)
anova(res1)

# simple model effects just in tail - this method of testing simple effects differs slightly from independent t-test outcomes (run below), because all the various sources of variance in the data are being accounted for by the model. This may or may not be preferable to the simple t-test approach depending on your question
contrasts(myrepeatedmeasures$variable) = c(0,1); contrasts(myrepeatedmeasures$variable)
res1a = lmer(value ~ variable * cond + (1|sub), data=myrepeatedmeasures)
summary(res1a)

# simple model effects just in body
contrasts(myrepeatedmeasures$variable) = c(1,0); contrasts(myrepeatedmeasures$variable)
res1b = lmer(value ~ variable * cond + (1|sub), data=myrepeatedmeasures)
summary(res1b)

# more complex model
res3 = lmer(value ~ variable * cond + (1 + variable|sub), data=myrepeatedmeasures)
anova(res3)

# most complex model 
contrasts(myrepeatedmeasures$variable) = c(1,-1); contrasts(myrepeatedmeasures$variable)
res2 = lmer(value ~ variable * cond + (1 + variable + cond|sub), data=myrepeatedmeasures)
anova(res2)
summary(res2)

# determine which model is best (take simplest when there is no sig Chi Sq)
anova(res1, res2, refit=FALSE)
anova(res1, res3, res2, refit=FALSE)
coef(res2)
